...
exports.generateWebsiteContent = (0, workflows_1.createStep)({
    ...
    inputSchema: zod_1.z.object({
        ...
        websiteGenerated: zod_1.z.boolean().describe('Whether the website was successfully generated'),
        componentsCreated: zod_1.z.array(zod_1.z.string()).describe('List of components that were created'),
        success: zod_1.z.boolean().describe('Whether the previous step completed successfully'),
    }),
    outputSchema: zod_1.z.object({
        ...
        projectName: zod_1.z.string().describe('The project name'),
        requirements: zod_1.z.string().describe('The original requirements report'),
        contentGenerated: zod_1.z.boolean().describe('Whether custom content was successfully generated'),
        stylingCompleted: zod_1.z.boolean().describe('Whether custom styling was completed'),
        success: zod_1.z.boolean().describe('Whether the step completed successfully'),
    }),
    execute: async (params) => {
        try {
            ...
            try {
                ...
                const { webBuilderAgent } = await Promise.resolve().then(() => __importStar(require('../workflows/webBuilder')));
...
function inferBusinessType(requirements) {
    ...
    if (lowerReq.includes('software') || lowerReq.includes('technology') || lowerReq.includes('development'))
        return 'technology';
    if (lowerReq.includes('consulting') || lowerReq.includes('advisory') || lowerReq.includes('strategy'))
        return 'consulting';
    if (lowerReq.includes('design') || lowerReq.includes('creative') || lowerReq.includes('art'))
        return 'creative';
    if (lowerReq.includes('health') || lowerReq.includes('medical') || lowerReq.includes('clinic'))
        return 'healthcare';
    if (lowerReq.includes('education') || lowerReq.includes('learning') || lowerReq.includes('school'))
        return 'education';
    if (lowerReq.includes('restaurant') || lowerReq.includes('food') || lowerReq.includes('dining'))
        return 'restaurant';
    if (lowerReq.includes('retail') || lowerReq.includes('shop') || lowerReq.includes('store'))
        return 'retail';
    return 'general';
}

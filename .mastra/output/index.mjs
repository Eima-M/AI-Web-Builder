...
const webAgent = new Agent({
  ...
  instructions: `
    ...
    **AFTER PROVIDING THE REPORT:** Ask the user if they would like to receive a copy of this report via email. If they say yes, use the email_report tool to send them the complete report.
`,
...
const llm$1 = openai(process.env.MODEL ?? "gpt-4.1");
...
}).then(createRepo).then(initializeFramework);

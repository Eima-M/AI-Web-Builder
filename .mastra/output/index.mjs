...
import { Tool, isVercelTool } from '@mastra/core/tools';
...
const emailWithAttachment = new Tool({
  ...
  inputSchema: z.object({
    ...
    filename: z.string().optional().describe("Name for the text file attachment (optional)")
  }),
  ...
  execute: async (params) => {
    try {
      ...
      const attachmentFilename = filename || `website-requirements-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.txt`;
      const htmlContent = `
        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background-color: #e8f4fd; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #007bff;">
            <h3 style="color: #007bff; margin-top: 0;">\u{1F4CE} Attachment Included</h3>
...
const generateReportFile = new Tool({
  ...
  execute: async (params) => {
    try {
      ...
      const defaultFilename = `website-requirements-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.txt`;
...
const parseRequirements = createStep({
  ...
  inputSchema: z.object({
    requirementsReport: z.string().describe("The complete website requirements report text")
  }),
  outputSchema: z.object({
    projectName: z.string().describe("Extracted or generated project name"),
  ...
  execute: async (params) => {
    try {
      ...
      if (!projectName && purpose) {
        projectName = generateConciseProjectName(purpose);
      }
      if (!projectName) {
        const timestamp = (/* @__PURE__ */ new Date()).toISOString().split("T")[0].replace(/-/g, "");
      ...
      return {
        ...
        extractedInfo: {
          ...
          targetAudience: targetAudience || void 0,
          businessObjectives: businessObjectives || void 0
        },
        success: true
      };
    } catch (error) {
...
function extractKeyTerms(text) {
  ...
  const words = text.toLowerCase().replace(/[^a-z0-9\s]/g, " ").split(/\s+/).filter((word) => word.length > 2 && !stopWords.includes(word)).filter((word) => word.length <= 12);
...
function sanitizeProjectName(name) {
  return name.toLowerCase().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/^-+|-+$/g, "").substring(0, 25).replace(/-+$/, "") || `project-${Date.now().toString().slice(-6)}`;
}

const createRepository = createStep({
  ...
  execute: async (params) => {
    try {
      ...
      const repoName = projectName.trim();
...
const initializeFramework = createStep({
  ...
  execute: async (params) => {
    try {
      const { repositoryUrl, repositoryName, projectName, requirements, success } = params.inputData || params.context || params.data || params.input || params;
      if (!success) {
...
const generateWebsiteStructure = createStep({
  ...
  execute: async (params) => {
    try {
      const { repositoryUrl, repositoryName, projectName, requirements, frameworkInitialized, success } = params.inputData || params.context || params.data || params.input || params;
      ...
      try {
        const { webBuilderAgent } = await Promise.resolve().then(function () { return webBuilder; });
...
async function generateWebsiteFiles(structureAnalysis, requirements, projectName) {
  ...
  const files = [
    ...
    },
    {
      ...
      content: generateServicesComponent()
    },
    {
      path: "src/components/Contact.tsx",
      content: generateContactComponent()
    },
    {
...
function parseRequirementsForWebsite(requirements) {
...
function generateAppComponent(extractedInfo, projectName) {
...
function generateAppCSS(extractedInfo) {
  const colorSchemes = {
  ...
  const colors = colorSchemes[extractedInfo.colorScheme] || colorSchemes.blue;
  return `.App {
  text-align: center;
}
...
function generateGlobalCSS(extractedInfo) {
  return `* {
  margin: 0;
  padding: 0;
...
function generateHeaderComponent(extractedInfo, projectName) {
...
function generateHeroComponent(extractedInfo, projectName) {
  return `import React from 'react';
...
function generateAboutComponent(extractedInfo) {
  return `import React from 'react';
...
function generateServicesComponent(extractedInfo) {
  return `import React from 'react';
...
function generateContactComponent(extractedInfo) {
  return `import React, { useState } from 'react';
...
function generateFooterComponent(extractedInfo, projectName) {
  return `import React from 'react';
      ...
      <div className="container" style={containerStyles}>
        ...
        <div style={footerBottomStyles}>
          <p style={copyrightStyles}>
            \xA9 {new Date().getFullYear()} ${projectName}. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
...
function generatePlaceholderSVG(type, color, width, height) {
...
function generateLogoSVG(projectName) {
...
const generateWebsiteContent = createStep({
  ...
  execute: async (params) => {
    try {
      const { repositoryUrl, repositoryName, projectName, requirements, websiteGenerated, componentsCreated, success } = params.inputData || params.context || params.data || params.input || params;
      ...
      const baseApiUrl = `https://api.github.com/repos/${owner}/${repositoryName}/contents`;
      const enhancedFiles = generateEnhancedComponents(parsedContent);
...
function parseGeneratedContent(content, requirements) {
  const parsed = {
    ...
    businessType: inferBusinessType$1(requirements),
...
function extractServices(content, requirements) {
  ...
  const businessType = inferBusinessType$1(requirements);
  switch (businessType) {
    case "technology":
      return [
...
function inferBusinessType$1(requirements) {
...
function inferIndustry(requirements) {
  const businessType = inferBusinessType$1(requirements);
  const industryMap = {
...
function generateEnhancedComponents(content, projectName) {
  return [
    {
      path: "src/components/Hero.tsx",
      content: generateEnhancedHero(content)
    },
    {
      path: "src/components/About.tsx",
    ...
    {
      path: "src/App.css",
      content: generateEnhancedCSS()
    }
  ];
}
function generateEnhancedHero(content, projectName) {
  return `import React from 'react';
...
function generateEnhancedAbout(content) {
  return `import React from 'react';
...
function generateEnhancedServices(content) {
  return `import React from 'react';
...
function generateEnhancedCSS(content) {
  return `.App {
  text-align: center;
}
...
const finalizeWebsite = createStep({
  id: "finalize-website",
  ...
  execute: async (params) => {
    try {
      const { repositoryUrl, repositoryName, projectName, requirements, contentGenerated, stylingCompleted, success } = params.inputData || params.context || params.data || params.input || params;
...
function generateFinalFiles(projectName, requirements, repositoryName) {
  return [
    {
      path: "README.md",
...
function generateREADME(projectName, requirements, repositoryName) {
  inferBusinessType(requirements);
  return `# ${projectName}
...
function inferBusinessType(requirements) {
  const lowerReq = requirements.toLowerCase();
  if (lowerReq.includes("software") || lowerReq.includes("technology") || lowerReq.includes("development")) return "technology";
  if (lowerReq.includes("consulting") || lowerReq.includes("advisory") || lowerReq.includes("strategy")) return "consulting";
  if (lowerReq.includes("design") || lowerReq.includes("creative") || lowerReq.includes("art")) return "creative";
...
}).then(parseRequirements).then(createRepository).then(initializeFramework).then(generateWebsiteStructure).then(generateWebsiteContent).then(finalizeWebsite);
webBuilderWorkflow.commit();

var webBuilder = /*#__PURE__*/Object.freeze({
  __proto__: null,
  webBuilderAgent: webBuilderAgent,
  webBuilderWorkflow: webBuilderWorkflow
});
